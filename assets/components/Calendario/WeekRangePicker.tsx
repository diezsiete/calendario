...
export function WeekRangePickerTrigger({ weekInfo, onNavigateWeek } : WeekRangePickerProps) {
    return <div className="btn-group week-range-picker-trigger">
        <button type="button" className="btn btn-outline-light navigate-week" onClick={() => onNavigateWeek?.('prev')}>
            <i className="bi bi-chevron-left"></i>
        </button>
        <button type="button" className="btn btn-outline-light week-label disabled">
            <i className="bi bi-calendar me-2"></i>
            {weekInfo.label}
        </button>
        <button type="button" className="btn btn-outline-light navigate-week" onClick={() => onNavigateWeek?.('next')}>
