...
export default function DropdownDb() {
    ...
    return <Dropdown title='Db' menuEnd btnClassName='btn-outline-success'>
        {dbs.map(db => <DropdownItemButton key={db.name} active={db.name === dbActive} onClick={() => handleDbSelect(db.name)}>
            {db.name}
        </DropdownItemButton>)}
        <DropdownDivider />
